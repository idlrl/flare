...
from flare.common.error_handling import check_type_error
from flare.common.replay_buffer import NoReplacementQueue, ReplayBuffer
from flare.framework.agent import AgentHelperBase, DataProcessorBase
...
class RLDataProcessor(DataProcessorBase):
    ...
    def process_prediction_inputs(self, inputs, states):
        assert inputs
        # add a new axis for batch size dimension
        new_inputs = {k: np.array([inputs[k]]) for k in inputs}
        data = dict(inputs=new_inputs)
        if states:
            # add a new axis for batch size dimension
            new_states = {k: np.array([states[k]]) for k in states}
